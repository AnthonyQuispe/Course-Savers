...
async function submitData(term: string, course: string) {
  const request = await fetch(
    `/api/searchResults?term=${term}&course=${course}`,
    {
      method: "GET",
      headers: {
...
export default function Results() {
  ...
  }, [status, getCourse, getEmail, getTerm, router]);
  ...
  return (
    <div className="pt-[44px]">
      ...
      <p className=" text-PrimaryPurp underline pl-6 pb-3 font-bold">
        My Schedule
      </p>
      <div>
        <ResultsScheduleElement
          scheduleData={userClasses}
          classToRegister={classAdded}
        />
      </div>
      <div>
        <div className="p-6 pt-[3rem]">
          {classAdded ? (
            <button
              ...
              className="bg-PrimaryPurp rounded-full w-full p-3 text-white text-xl font-extrabold"
            >
              Register
            </button>
          ) : (
        ...
        <div className="p-6">
          <h2 className=" text-DarkPurp text-xl font-extrabold pb-3">
            SELECT CLASSES
          </h2>
          <span>Results for: </span>
          <span className=" italic">
            {getCourse === "" ? "All Courses" : getCourse} {/* - {getTerm} */}
          </span>
        </div>
        <section>
          <ResultsClassElement
            resultData={foundClasses}
            addToRegisterClass={addToRegisterClass}
          />
        </section>
      </div>
    </div>
